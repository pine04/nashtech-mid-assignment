...
const useAddBooks = () => {
    ...
    const mutation = useMutation({
        ...
        onSuccess: (book: Book) => {
            ...
            toast.success(`Created new book with ID ${book.id}.`);
        },
        onError: (error: AxiosError<any, any>) => {
            toast.error(error.response?.data.detail || `An error happened while creating book.`);
        }
    });

    return {
        addBook: mutation.mutateAsync
    }
}
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <CategorySelector initialSearchQuery={""} value={field.value} setValue={(id) => form.setValue("categoryId", id)} />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    )}
